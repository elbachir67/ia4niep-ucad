...
pathwaySchema.methods.updateProgress = async function() {
  ...
  if (this.progress > 0) {
    ...
    this.estimatedCompletionDate = new Date(Date.now() + estimatedRemainingTime);
  }
...
pathwaySchema.index({ userId: 1, status: 1 });
pathwaySchema.index({ goalId: 1 });
pathwaySchema.index({ 'moduleProgress.completed': 1 });
