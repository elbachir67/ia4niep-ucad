...
router.get('/pathways/:pathwayId/modules/:moduleId/quiz', auth, async (req, res) => {
  try {
    const { pathwayId, moduleId } = req.params;
    logger.info(`Fetching quiz for pathway ${pathwayId} and module ${moduleId}`);
...
// Submit quiz attempt
router.post('/pathways/:pathwayId/modules/:moduleId/quiz/submit', auth, async (req, res) => {
  try {
    const { pathwayId, moduleId } = req.params;
    const { score, answers, totalTimeSpent, categoryScores, recommendations } = req.body;
    ...
    if (moduleIndex > -1) {
      ...
      const allResourcesCompleted = pathway.moduleProgress[moduleIndex].resources
        .every(r => r.completed);
      ...
      pathway.progress = Math.round(
        ...
        pathway.moduleProgress.length) * 100
      );
...
router.post('/pathways/:pathwayId/modules/:moduleId/quiz/reset', auth, async (req, res) => {
  try {
    const { pathwayId, moduleId } = req.params;
...
router.get('/pathways/:pathwayId/modules/:moduleId/quiz/attempts', auth, async (req, res) => {
  try {
    const { pathwayId, moduleId } = req.params;
