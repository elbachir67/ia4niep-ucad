...
interface PathwayAdaptation {
  title: string;
  description: string;
  type: 'add_resource' | 'remove_resource' | 'change_order' | 'add_module' | 'simplify';
  resourceId?: string;
  moduleId?: string;
}
...
export function updateConceptMastery(
  ...
  const avgTimePerQuestion = responses.reduce((sum, r) => sum + r.timeSpent, 0) / responses.length;
  ...
  if (existingMastery) {
    return {
      conceptId,
      conceptName,
      masteryLevel: Math.round((existingMastery.masteryLevel * 0.3) + (masteryLevel * 0.7)), // Weight new results more
      confidence: Math.round((existingMastery.confidence * 0.3) + (confidence * 0.7)),
      lastAssessed: new Date(),
      weakPoints: [...new Set([...weakPoints, ...existingMastery.weakPoints])],
      strongPoints: [...new Set([...strongPoints, ...existingMastery.strongPoints])]
    };
  }
