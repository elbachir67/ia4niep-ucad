...
export function filterGoalsByUserProfile(
  ...
  const scoredGoals = goals.map(goal => {
    ...
    const hasRequiredSkills = goal.prerequisites.every(prereq => {
      ...
      if (prereq.category.toLowerCase().includes('programming')) {
        return isLevelSufficient(programmingLevel, getMaxSkillLevel(prereq.skills));
      }
      return true;
    });
  ...
  const sortedGoals = scoredGoals.sort((a, b) => 
...
export function filterGoalsBySearch(
  ...
  return goals.filter(goal => {
    const matchesSearch = 
      ...
      goal.careerOpportunities?.some(o => 
        o.title.toLowerCase().includes(searchLower) ||
        o.description.toLowerCase().includes(searchLower)
      );

    const matchesCategory = !category || category === 'all' || goal.category === category;
    const matchesDifficulty = !difficulty || difficulty === 'all' || goal.level === difficulty;
...
function isLevelSufficient(userLevel: GoalDifficulty, requiredLevel: GoalDifficulty): boolean {
