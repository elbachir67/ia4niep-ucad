...
function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated) {
      const redirectUrl = localStorage.getItem('redirectAfterLogin') || '/goals';
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0A0A0F] py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 glass-card p-8 rounded-xl">
        ...
        <div className="border-t border-gray-700 pt-6">
          <h3 className="text-sm font-medium text-gray-400 mb-3">Comptes de démonstration :</h3>
          <div className="space-y-2">
            {testUsers.map((user, index) => (
              ...
              >
                ...
                <div className="text-gray-500 text-xs">Mot de passe: {user.password}</div>
              </button>
            ))}
          </div>
        </div>
