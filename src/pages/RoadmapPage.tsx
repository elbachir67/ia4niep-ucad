...
function StepCard({ step, index, isAdmin }: { step: Step; index: number; isAdmin: boolean }) {
      ...
      <div className="p-6">
        ...
        {isExpanded && (
          <div className="mb-6 animate-fadeIn">
            <h4 className="text-lg font-semibold text-gray-800 mb-2">Description détaillée</h4>
            ...
            {step.prerequisites && step.prerequisites.length > 0 && (
              <div className="mt-4">
                <h5 className="text-md font-semibold text-gray-800 mb-2">Prérequis</h5>
            ...
            {step.learningObjectives && step.learningObjectives.length > 0 && (
              <div className="mt-4">
                <h5 className="text-md font-semibold text-gray-800 mb-2">Objectifs d'apprentissage</h5>
                <ul className="list-disc list-inside text-gray-600">
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {resources.map((resource) => {
                ...
                return (
                  <div key={resource._id} className="flex items-center justify-between p-3 rounded-lg border border-gray-200 bg-gray-50 hover:bg-gray-100 transition-colors">
                    <a
                      href={resource.url}
                      target="_blank"
                    ...
                    >
                      <span className={`p-2 rounded-md ${typeConfig.bg} mr-3 group-hover:scale-110 transition-transform`}>
                    ...
                    <div className="flex items-center space-x-2">
                      <span className={`px-3 py-1 text-xs font-medium rounded-full border ${levelStyle.bg} ${levelStyle.text} ${levelStyle.border}`}>
                        {levelStyle.label}
                      </span>
                      {resource.isPremium && (
            ...
            <div className="text-sm text-gray-500">Aucune ressource disponible</div>
          )}
        </div>
      </div>
    </div>
...
function RoadmapPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!sectionsResponse.ok) throw new Error('Erreur lors du chargement des sections');
        const sectionsData = await sectionsResponse.json();
        setSections(sectionsData);

        // Initialize expanded state for all sections
        const initialExpandedState = sectionsData.reduce((acc: any, section: Section) => {
        ...
        if (!stepsResponse.ok) throw new Error('Erreur lors du chargement des étapes');
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
      <h1 className="text-4xl font-bold text-gray-900 mb-12">Parcours d'Apprentissage</h1>
      
      <div className="space-y-16">
        {sections.map((section) => {
          const sectionSteps = steps.filter(step => step.section._id === section._id);
          const isExpanded = expandedSections[section._id];
          
          return (
            <div key={section._id} className="bg-white rounded-lg shadow-sm overflow-hidden">
              <button
              ...
              >
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-sm text-gray-500">
                    {sectionSteps.length} étape{sectionSteps.length > 1 ? 's' : ''}
                  </span>
                  {isExpanded ? (
