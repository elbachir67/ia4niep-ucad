...
import { filterGoalsByUserProfile, filterGoalsBySearch } from '../utils/goalFilters';
...
function GoalsExplorerPage() {
  ...
  const [selectedDifficulty, setSelectedDifficulty] = useState<GoalDifficulty | 'all'>('all');
  ...
  useEffect(() => {
    ...
    if (!hasCompletedAssessment) {
      navigate('/assessment', { 
        state: { 
          returnPath: '/goals',
          message: "Pour accéder aux objectifs personnalisés, une courte évaluation est nécessaire."
        }
      });
      return;
    }
    ...
    const fetchData = async () => {
      try {
        ...
        const goalsResponse = await fetch(`${api.goals}?${new URLSearchParams({
        ...
        const { recommended, others } = filterGoalsByUserProfile(goalsData, profileData);
    ...
    <div className="min-h-screen bg-[#0A0A0F] py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-100">Objectifs d'Apprentissage</h1>
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <select
                ...
                onChange={(e) => setSelectedDifficulty(e.target.value as GoalDifficulty | 'all')}
                className="px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:ring-2 focus:ring-purple-500"
              >
        ...
        {filteredGoals.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-400">Aucun objectif ne correspond à vos critères de recherche.</p>
          </div>
        )}
      </div>
    </div>
