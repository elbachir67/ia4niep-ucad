...
import { calculateDetailedScore, generateRecommendations } from '../utils/scoringSystem';

function AssessmentPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const { user, isAuthenticated, checkAssessmentStatus } = useAuth();
  const [currentStep, setCurrentStep] = useState<'intro' | 'math' | 'programming' | 'domain' | 'quiz' | 'results'>('intro');
  ...
  const handleStepTransition = async (input: string) => {
    switch (currentStep) {
      ...
      case 'math':
        const mathLevel = input.toLowerCase().includes('débutant') ? 'beginner' :
                         input.toLowerCase().includes('avancé') ? 'advanced' : 'intermediate';
        setUserProfile(prev => ({ ...prev, mathLevel }));
        setCurrentStep('programming');
        setTimeout(() => {
          addBotMessage(
            "Et quel est votre niveau en programmation ?",
            ["Débutant", "Intermédiaire", "Avancé"]
          );
        }, 1000);
        break;
      ...
      case 'programming':
        const progLevel = input.toLowerCase().includes('débutant') ? 'beginner' :
                         input.toLowerCase().includes('avancé') ? 'advanced' : 'intermediate';
        setUserProfile(prev => ({ ...prev, programmingLevel: progLevel }));
        setCurrentStep('domain');
        setTimeout(() => {
          addBotMessage(
            "Quel domaine de l'IA vous intéresse le plus ?",
      ...
      case 'domain':
        ...
        const selectedDomain = Object.entries(domainMap)
          .find(([key]) => input.includes(key))?.[1] || 'ml';
  ...
  const loadQuestions = async (domain: string) => {
    setLoading(true);
    try {
      const response = await fetch(`${api.assessments}/questions?domain=${domain}`, {
        headers: {
  ...
  const handleQuizComplete = async (score: number, responses: any[]) => {
    ...
    const recommendations = generateRecommendations(categoryScores, userProfile);
      ...
      try {
        ...
        addBotMessage(
          ...
          recommendations.map(rec => 
            `${rec.category}: ${rec.score}% - Niveau ${rec.level}\n${rec.recommendations.join('\n')}`
          ).join('\n\n') +
    ...
    <div className="min-h-screen bg-[#0A0A0F] py-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-100">Évaluation Personnalisée</h1>
