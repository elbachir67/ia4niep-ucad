...
import { Clock, CheckCircle, XCircle, ArrowRight, Loader2, Brain, AlertCircle } from 'lucide-react';
...
function ConceptAssessmentPage() {
  ...
  useEffect(() => {
    const fetchConceptAndQuestions = async () => {
      ...
      try {
        ...
        const questionsResponse = await fetch(`${api.concepts}/${conceptId}/assessment`, {
          headers: {
            'Authorization': `Bearer ${user.token}`,
            'Content-Type': 'application/json'
          }
  ...
  const handleQuizComplete = async (score: number, responses: any[]) => {
    ...
    try {
      const response = await fetch(`${api.concepts}/${conceptId}/assessment/submit`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${user.token}`,
          'Content-Type': 'application/json'
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="glass-card rounded-xl p-6 mb-8">
          <div className="flex items-center space-x-4 mb-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-100">{concept.name}</h1>
              <p className="text-gray-400">{concept.description}</p>
            </div>
          </div>
          <div className="flex items-center space-x-4">
            <span className={`px-3 py-1 rounded-full text-sm font-medium ${
              concept.level === 'basic' ? 'bg-green-500/20 text-green-400' :
              concept.level === 'intermediate' ? 'bg-blue-500/20 text-blue-400' :
              'bg-purple-500/20 text-purple-400'
            }`}>
              {concept.level === 'basic' ? 'Débutant' : 
               concept.level === 'intermediate' ? 'Intermédiaire' : 'Avancé'}
            </span>
        ...
        <div className="glass-card rounded-xl p-6 mb-8">
          <h2 className="text-xl font-bold text-gray-100 mb-4">Évaluation du concept</h2>
          <p className="text-gray-400 mb-6">
            Cette évaluation vous permettra de mesurer votre compréhension du concept et d'adapter votre parcours d'apprentissage en conséquence.
          </p>
          <QuizComponent 
            questions={questions} 
