...
function GoalDetailPage() {
  ...
  const [generatedPathwayId, setGeneratedPathwayId] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchGoal = async () => {
      ...
      try {
        ...
        const initialExpandedState = data.modules?.reduce((acc: any, _: any, index: number) => {
          acc[index] = index === 0;
          return acc;
        }, {}) || {};
  ...
  const generatePathway = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Erreur lors de la génération du parcours');
      }
      ...
      if (error instanceof Error && error.message.includes('existe déjà')) {
        toast.error('Un parcours pour cet objectif existe déjà. Consultez votre tableau de bord pour y accéder.', {
          duration: 5000,
          icon: <AlertCircle className="text-red-500" />
        });
        setTimeout(() => navigate('/dashboard'), 2000);
      } else {
        toast.error('Impossible de générer le parcours. Veuillez réessayer plus tard.', {
          duration: 4000
        });
      }
    } finally {
      ...
      <div className="bg-gray-900 rounded-xl p-8 max-w-md w-full mx-4">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-400 mb-6">
            Votre parcours personnalisé a été créé. Que souhaitez-vous faire maintenant ?
          </p>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="glass-card rounded-xl p-8 mb-8 bg-gradient-to-br from-purple-500/10 via-blue-500/10 to-purple-500/10">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-100 mb-4">{goal.title}</h1>
              ...
              <div className="flex flex-wrap gap-4 items-center">
                ...
                <div className="flex items-center">
                  <span className={`px-4 py-2 rounded-lg text-sm font-medium ${levelConfig[goal.level].bg} ${levelConfig[goal.level].color} border ${levelConfig[goal.level].border}`}>
              ...
              {goal.prerequisites.map((prereq, index) => (
                <div key={index} className="bg-gray-800/50 rounded-lg p-4">
                  <h3 className="text-lg font-semibold text-gray-200 mb-2">
                    {prereq.category === 'math' ? 'Mathématiques' :
                     prereq.category === 'programming' ? 'Programmation' :
                     prereq.category}
                  </h3>
                    ...
                    {prereq.skills.map((skill, skillIndex) => (
                      <div
                        ...
                        className={`px-3 py-1 rounded-full text-sm ${
                          skill.level === 'basic' ? 'bg-green-500/20 text-green-400' :
            ...
            <div key={index} className="glass-card rounded-xl p-6">
              ...
              {expandedModules[index] && (
                <div className="mt-6 space-y-6 animate-fadeIn">
                  ...
                  <div className="space-y-4">
                    <h4 className="text-lg font-medium text-gray-300">Ressources</h4>
                    <div className="grid gap-4">
                      {module.resources?.map((resource, resourceIndex) => {
                        const typeConfig = resourceTypeConfig[resource.type as ResourceType];
                        ...
                        return (
                          ...
                          >
                            <div className={`p-2 rounded-lg ${typeConfig.bg} mr-4 group-hover:scale-110 transition-transform`}>
                  ...
                  {module.validationCriteria && (
                    <div className="space-y-4">
                      ...
                      <ul className="space-y-2">
                        {module.validationCriteria.map((criteria, criteriaIndex) => (
                          <li
                            key={criteriaIndex}
