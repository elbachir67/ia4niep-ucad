...
interface AdaptiveRecommendationsProps {
  ...
  onRecommendationAction: (index: number, action: 'start' | 'skip' | 'complete') => void;
}
...
const AdaptiveRecommendations: React.FC<AdaptiveRecommendationsProps> = ({ 
  ...
  const handleAction = async (index: number, action: 'start' | 'skip' | 'complete') => {
    try {
  ...
  return (
    <div className="space-y-4">
      ...
      {recommendations.map((recommendation, index) => (
        <div
          key={index}
          className={`glass-card rounded-xl p-6 border-l-4 ${getPriorityStyles(recommendation.priority)}`}
        >
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <h3 className="text-lg font-semibold text-gray-200">
                  {recommendation.type === 'resource' ? 'Ressource recommandée' :
                   recommendation.type === 'practice' ? 'Exercice pratique' :
                   'Révision suggérée'}
                </h3>
