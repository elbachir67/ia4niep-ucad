...
export const assessmentService = {
  ...
  getAssessmentQuestions: async (domain?: string) => {
    const response = await apiClient.get(`${api.assessments}/questions${domain ? `?domain=${domain}` : ''}`);
  ...
  submitAssessment: async (assessmentData: {
    ...
    const response = await apiClient.post(`${api.assessments}/submit`, assessmentData);
  ...
  getConceptAssessment: async (conceptId: string) => {
    const response = await apiClient.get(`${api.concepts}/${conceptId}/assessment`);
    return response.data;
  },

  // Soumettre les résultats d'une évaluation de concept
  submitConceptAssessment: async (conceptId: string, assessmentData: {
    score: number;
    responses: any[];
  }) => {
    const response = await apiClient.post(`${api.concepts}/${conceptId}/assessment/submit`, assessmentData);
    return response.data;
  }
};