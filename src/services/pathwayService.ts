...
export const pathwayService = {
  ...
  generatePathway: async (goalId: string) => {
    const response = await apiClient.post(`${api.pathways}/generate`, { goalId });
  ...
  updateModuleProgress: async (pathwayId: string, moduleIndex: number, data: {
    resourceId?: string;
    completed?: boolean;
  }) => {
    try {
  ...
  updateRecommendation: async (pathwayId: string, recommendationIndex: number, action: 'start' | 'skip' | 'complete') => {
    try {
      const response = await apiClient.put(
  ...
  getProgressData: async (pathwayId: string) => {
    try {
      const response = await apiClient.get(`${api.pathways}/${pathwayId}/progress`);
