...
export const quizService = {
  ...
  getQuiz: async (pathwayId: string, moduleId: string) => {
    const response = await apiClient.get(`${api.pathways}/${pathwayId}/modules/${moduleId}/quiz`);
    return response.data as ModuleQuiz;
  },

  // Soumettre les résultats d'un quiz
  submitQuiz: async (pathwayId: string, moduleId: string, quizData: {
    score: number;
    answers: any[];
  ...
  resetQuiz: async (pathwayId: string, moduleId: string) => {
    const response = await apiClient.post(`${api.pathways}/${pathwayId}/modules/${moduleId}/quiz/reset`);
    return response.data;
  },

  // Récupérer l'historique des tentatives de quiz
  getQuizAttempts: async (pathwayId: string, moduleId: string) => {
    const response = await apiClient.get(`${api.pathways}/${pathwayId}/modules/${moduleId}/quiz/attempts`);
    return response.data;
  }
};